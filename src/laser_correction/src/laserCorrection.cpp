...
public:
    ...
    LidarMotionCalibrator(tf::TransformListener* tf)
    {
        ...
        scan_pub_ = nh_.advertise<sensor_msgs::LaserScan>("scan_correction", 10);
    }
    ...
    void ScanCallBack(const sensor_msgs::LaserScanConstPtr& scan_msg)
    {
        ...
        {   
            ...
            double lidar_angle = laserScanMsg.angle_min + i * laserScanMsg.angle_increment;
        ...
        scan_correction = laserScanMsg;
        for(int i = beamNum - 1; i >= 0;i--)
        {
            scan_correction.ranges[beamNum - i -1] = ranges[i];
        }

        scan_pub_.publish(scan_correction);
    ...
    */
                      ...
                      tf::TransformListener * tf_)
    {
        ...
        robot_pose.frame_id_ = "laser";
        robot_pose.stamp_ = dt;   //设置为ros::Time()表示返回最近的转换关系

        // get the global pose of the robot
        try
        {
            if(!tf_->waitForTransform("odom", "laser", dt, ros::Duration(0.5)))             // 0.15s 的时间可以修改
            {
                           ...
                           tf::TransformListener * tf_)
    {
        ...
        for(int i = 0; i < beamNumber; i++)
        {
            ...
            if(mid_time - start_time > interpolation_time_duration || (i == beamNumber - 1))
            {
                ...
                Lidar_MotionCalibration(frame_base_pose, frame_start_pose, frame_mid_pose, ranges, angles, start_index, interp_count);
...
private:
    ...
    sensor_msgs::LaserScan scan_correction;
};
